    ...
    while open_heap:
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                # 计算到障碍物的最小有效距离
                min_distance = get_min_obstacle_distance(cx_phys, cy_phys, obstacles, formation_radius, vehicle_radius)
                # 计算权重（距离越小，权重越大）
                epsilon = 0.1  # 防止除以零
                weight = 1 + 1.0 / (min_distance + epsilon)
                # 调整移动代价
