    ...
    while open_heap:
        ...
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                ...
                # --- 计算移动代价权重（含边界惩罚）---
                # 获取物理坐标
                cx_phys = neighbor[0] * grid_size + grid_size / 2
                cy_phys = neighbor[1] * grid_size + grid_size / 2

                # 计算到障碍物的最小有效距离
                obstacle_min_distance = get_min_obstacle_distance(cx_phys, cy_phys, obstacles, formation_radius, vehicle_radius)

                # 计算到边界的有效距离
                dx_left = cx_phys - x_min_phys
                dx_right = x_max_phys - cx_phys
                dy_bottom = cy_phys - y_min_phys
                dy_top = y_max_phys - cy_phys
                boundary_distance_raw = min(dx_left, dx_right, dy_bottom, dy_top)
                margin = formation_radius + vehicle_radius
                boundary_effective_distance = boundary_distance_raw - margin
                boundary_effective_distance = max(boundary_effective_distance, 0)  # 确保不小于0

                # 总的最小有效距离取障碍物和边界的较小者
                total_min_distance = min(obstacle_min_distance, boundary_effective_distance)

                # 计算权重（距离越小，权重越大）
                epsilon = 0.1  # 防止除以零
                weight = 1 + 1.0 / (total_min_distance + epsilon)
                move_cost = math.hypot(dx * grid_size, dy * grid_size) * weight
                # ---------------------------------
...
def is_blocked(x_grid, y_grid, obstacles, formation_radius, vehicle_radius):
    """动态计算网格中心物理坐标，支持圆形和凸多边形障碍物及边界检查"""
    ...
    # 检查是否超出物理边界，考虑安全边距
    (x_min_phys, x_max_phys), (y_min_phys, y_max_phys) = grid_boundary
    if (cx - margin < x_min_phys) or (cx + margin > x_max_phys) or (cy - margin < y_min_phys) or (cy + margin > y_max_phys):
        return True
...
def get_min_obstacle_distance(cx, cy, obstacles, formation_radius, vehicle_radius):
    """计算物理坐标到所有障碍物的最小有效距离（考虑安全边距）"""
    min_distance = float('inf')
    margin = formation_radius + vehicle_radius
    for obs in obstacles:
        if isinstance(obs, tuple) and len(obs) == 3:  # 圆形障碍物
            ...
            effective_radius = r + margin
            effective_distance = distance_to_center - effective_radius
            ...
            effective_distance = min_poly_distance - margin
            min_distance = min(min_distance, effective_distance)
    return max(min_distance, 0) if min_distance != float('inf') else float('inf')
