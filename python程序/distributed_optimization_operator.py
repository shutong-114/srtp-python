...
class DistributedFormationOptimizer:
    ...
    '''
    # ---- 以下为 snapshot-based helper：把它们也追加进类中 ----

    def _compute_update_robot1_from_snap(self, initial_positions, LF_vertices, k, positions_snap, estimates_snap, dual_snap, L_snap):
        i = 1
        p1 = positions_snap[1].copy()
        p12 = estimates_snap[1].get('p12', positions_snap[1].copy())
        y1 = dual_snap[1].get('y1', np.zeros_like(p1))
        z1 = dual_snap[1].get('z1', np.zeros_like(p1))
        w12 = dual_snap[1].get('w12', np.zeros_like(p12))
        xi1 = initial_positions[i-1]

        # 邻居一致项（基于 snapshot）
        neighbors = np.where(L_snap[i-1,:] != 0)[0] + 1
        consensus_p1 = np.zeros_like(p1)
        consensus_p12 = np.zeros_like(p12)
        for j in neighbors:
            if j == 1:
                p_j1 = positions_snap[1]
                p_j2 = estimates_snap[1].get('p12', positions_snap[1])
            elif j == 2:
                p_j1 = estimates_snap[2].get('p21', positions_snap[2])
                p_j2 = positions_snap[2]
            else:
                p_j1 = estimates_snap[j].get('p_i1', positions_snap[j])
                p_j2 = estimates_snap[j].get('p_i2', positions_snap[j])
            weight = -L_snap[i-1, j-1]
            consensus_p1 += weight * (p1 - p_j1)
            consensus_p12 += weight * (p12 - p_j2)

        p1_new = self._project_to_region(
            p1 - self.sigma * ((p1 - xi1) + y1 + (p1 - self.anchor_b - p12) + z1 + consensus_p1), LF_vertices
        )

        p12_new = self._project_to_region(
            p12 - self.sigma * (-y1 - (p1 - p12 - self.anchor_b) + w12 + consensus_p12), LF_vertices
        )
        ...
        return {'p1': p1_new, 'p12': p12_new, 'y1': y1_new, 'z1': z1_new, 'w12': w12_new}


    # ---------- 替换：robot2 snapshot helper（修正符号与对偶变量） ----------
    def _compute_update_robot2_from_snap(self, initial_positions, LF_vertices, k, positions_snap, estimates_snap, dual_snap, L_snap):
        i = 2
        p2 = positions_snap[2].copy()
        p21 = estimates_snap[2].get('p21', positions_snap[2].copy())
        y2 = dual_snap[2].get('y2', np.zeros_like(p2))
        z21 = dual_snap[2].get('z21', np.zeros_like(p2))
        w2 = dual_snap[2].get('w2', np.zeros_like(p21))
        xi2 = initial_positions[i-1]

        neighbors = np.where(L_snap[i-1,:] != 0)[0] + 1
        consensus_p2 = np.zeros_like(p2)
        consensus_p21 = np.zeros_like(p21)
        for j in neighbors:
            if j == 1:
                p_j1 = positions_snap[1]
                p_j2 = estimates_snap[1].get('p12', positions_snap[1])
                ...
                p_j2 = estimates_snap[j].get('p_i2', positions_snap[j])
            weight = -L_snap[i-1, j-1]
            # 注意：p2 对应邻居 j 的 p_j2，一致性项按原注释版计算
            if p_j2 is not None:
                consensus_p2 += weight * (p2 - p_j2)
            if p_j1 is not None:
                consensus_p21 += weight * (p21 - p_j1)

        # 恢复原注释版的符号与对偶变量配对
        p2_new = self._project_to_region(
            p2 - self.sigma * ((p2 - xi2) + y2 + (p2 + self.anchor_b - p21) + w2 + consensus_p2),
            LF_vertices
        )

        p21_new = self._project_to_region(
            p21 - self.sigma * (-y2 - (p2 - p21 + self.anchor_b) + z21 + consensus_p21),
            LF_vertices
        )
        ...
        return {'p2': p2_new, 'p21': p21_new, 'y2': y2_new, 'z21': z21_new, 'w2': w2_new}


    # ---------- 替换：robot_i snapshot helper（修正 y_i 更新与 B_i 相关项） ----------
    def _compute_update_robot_i_from_snap(self, i, initial_positions, LF_vertices, k, positions_snap, estimates_snap, dual_snap, L_snap, B_matrices):
        p_i = positions_snap[i].copy()
        p_i1 = estimates_snap[i].get('p_i1', positions_snap[i].copy())
        p_i2 = estimates_snap[i].get('p_i2', positions_snap[i].copy())
        y_i = dual_snap[i].get('y_i', np.zeros_like(p_i))
        z_i1 = dual_snap[i].get('z_i1', np.zeros_like(p_i))
        w_i2 = dual_snap[i].get('w_i2', np.zeros_like(p_i))
        xi = initial_positions[i-1]

        neighbors = np.where(L_snap[i-1,:] != 0)[0] + 1
        consensus_pi = np.zeros_like(p_i)
        consensus_pi1 = np.zeros_like(p_i1)
        consensus_pi2 = np.zeros_like(p_i2)
        for j in neighbors:
            if j == 1:
                p_j1 = positions_snap[1]
                p_j2 = estimates_snap[1].get('p12', positions_snap[1])
            ...
            weight = -L_snap[i-1, j-1]
            if p_j1 is not None:
                consensus_pi += weight * (p_i - p_j1)
                consensus_pi1 += weight * (p_i1 - p_j1)
            if p_j2 is not None:
                consensus_pi2 += weight * (p_i2 - p_j2)
        ...
        p_i_new = self._project_to_region(
            p_i - self.sigma * ((p_i - xi) + self.M_i[2].T @ y_B_term),
            LF_vertices
        )
        p_i1_new = self._project_to_region(
            p_i1 - self.sigma * (((self.M_i[i].T - self.M_i[2].T) @ y_B_term) + z_i1 + consensus_pi1),
            LF_vertices
        )
        p_i2_new = self._project_to_region(
            p_i2 - self.sigma * ((-self.M_i[i].T @ y_B_term) + w_i2 + consensus_pi2),
            LF_vertices
        )

        p_tilde_i_new = np.hstack([p_i_new, p_i1_new, p_i2_new])
        # 关键：y_i_new 恢复为论文/原注释版中 B_i @ p_tilde_i_new 的形式
        y_i_new = y_i + self.eta*(B_i @ p_tilde_i_new)
        z_i1_new = z_i1 + self.eta*consensus_pi
        w_i2_new = w_i2 + self.eta*consensus_pi2

        return {'p_i': p_i_new, 'p_i1': p_i1_new, 'p_i2': p_i2_new, 'y_i': y_i_new, 'z_i1': z_i1_new, 'w_i2': w_i2_new}
    ...
    def optimize_distributed(self, initial_positions, LF_vertices, adjacency_matrix, b):
        """
        Synchronous (Jacobi-style) implementation:
        - take snapshots of positions, estimates, dual_vars, L
        - compute updates for all robots using snapshot (no immediate writes)
        - after computing all, write back new values at once
        """
        # 这些方法应在你的类里已有
        self.set_communication_topology(adjacency_matrix)
        self.set_anchor_constraint(b)
        ...
        for k in range(self.max_iter):
            ...
            # 计算每个机器人的更新（不写回）
            updates_new[1] = self._compute_update_robot1_from_snap(initial_positions, LF_vertices, k,
                                                                positions_snap, estimates_snap, dual_snap, L_snap)
            updates_new[2] = self._compute_update_robot2_from_snap(initial_positions, LF_vertices, k,
                                                                positions_snap, estimates_snap, dual_snap, L_snap)

            for i in range(3, self.m+1):
                updates_new[i] = self._compute_update_robot_i_from_snap(i, initial_positions, LF_vertices, k,
